...
/*
  1.Problem->Two diff user doesn't get same seat
  Optimistic Lock-> Multiple users are allowed to read same data,and version is maintained for it, when
  anybody wants to update it ,first it will check if version has been changed.If it is changed ,it will not
  update it. If the version is not changed,it will lock ,and it will update the data and changed the version,release the lock.
  Pessimistic Lock -> lock during reading data ,then update and release lock
     In this case ,when the user reads a data, other user will not able to read it.

    For movie booking Optimistic lock is suitable because when a user reads data, while selecting seat user
    may select different seats,so locking all seats will be not a good decision.
   2. When  a user select seat for booking , it should go on hold for 10 minutes.
      We can use redis for that. Redis can be used with locks and we can also give timer to it.
 */
